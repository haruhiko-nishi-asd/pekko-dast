...
/** Browser-driven, LLM-directed click exploration: from a live page, repeatedly
  ...
  * screen it, click it, and harvest the state it reveals — extending coverage
  * to content behind JS buttons that a link/form crawl never reaches.
  *
  ...
  * guards and the [[ClickOp.precheck]] gating are exercised without a real
  * page. Termination is guaranteed three ways, as in [[NavLoop]]: a click
  * budget (each iteration spends one), a cycle guard (re-picking the SAME
  * control — by identity, not its re-stamped id — on the same page ends the
  * loop), and a dry counter (clicks that reveal no new url+DOM state). Every
  * click clears [[ConsentGate]] and the [[ClickGuard]] destructive floor; a
  * denied, off-menu, or not-found choice is skipped (counted dry), never fired.
  */
object ClickLoop:
  ...
  trait Effects:
    ...
    /** Scroll to load lazily-rendered content; `Some((url, html))` is the new
      * state if more loaded, `None` if already at the bottom / nothing changed.
      * Defaults to a no-op so non-browser callers need not implement it.
      */
    def scroll(): Future[Option[(String, String)]] = Future.successful(None)
  ...
  /** Explore from a live page. `maxClicks` bounds the iterations (an upper
    * bound on clicks). `history0` seeds the reached-page set, so a caller can
    * thread it across pages to keep the set distinct.
    */
  def explore(
      seedUrl: String,
      seedHtml: String,
    ...
    def loop(
      ...
      if clicksLeft <= 0 then Future.successful(Outcome(acc2, performed))
        ...
        effects.enumerate().flatMap { targets =>
          ...
          val advanced = (keys -- seenControls).nonEmpty ||
          ...
          if dry2 >= DryLimit then Future.successful(Outcome(acc2, performed))
            ...
            planner(url, targets, acc2.map(_._1).distinct, clickedHere)
              .flatMap { step =>
                step match
                  case ClickStep.Done => Future
                      .successful(Outcome(acc2, performed))
                  case ClickStep.Scroll =>
                    // Scroll spends from the budget (clicksLeft) so it cannot
                    // loop; progress is decided at the next enumeration (rows
                    // loaded -> new control keys -> dry resets, else dry climbs).
                    effects.scroll().flatMap { reached =>
                  ...
                  case ClickStep.Click(id) => targets.find(_.id == id) match
                      case None =>
                        log
                          .info("Click {} off the menu on {}; skipping", id, url)
                        loop(
                          url,
                          html,
                          visited,
