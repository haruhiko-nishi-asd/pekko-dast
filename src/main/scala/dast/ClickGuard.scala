...
/** The deterministic safety floor for LLM-directed clicking — the click
  ...
  * clicking the wrong control can delete, pay, or log out. So the model's choice
  * is necessary, never sufficient: a control whose accessible name or role
  * matches the destructive deny-list is refused, as is a disabled control. The
  * deny-list is the hard floor, so a model misjudgement still cannot fire a
...
object ClickGuard:
  ...
  private val denyRegex: scala.util.matching.Regex =
    ("(?i)\\b(" + denyPatterns.map(java.util.regex.Pattern.quote).mkString("|") +
      ")\\b").r
