...
**ツールの確認方法。** ここはモデルが本当に要となる唯一の箇所です。ログイン（認証付きスキャンの例外規定）から、スキャナは 2 つの方法でオブジェクトを持つページに到達します。`AuthCrawl`（決定的な同一ホストのリンククロール）と `NavLoop`（モデルがホップごとに索引付き要素から 1 つだけ選ぶ。リンク追従かフォーム送信で、クロールでは届かない一覧に到達する）。すべての送信は `ActionGuard` を通ります。GET は常に許可、POST はモデルが安全と判定し **かつ** 破壊的パターンの拒否リストを通過した場合のみ（モデルの判定は必要条件だが十分条件ではない）。SPA 対象では、リンク/フォームのクロールでは到達できないボタン依存の UI もブラウザが**クリック**します。`ClickTargetScanOp` が表示中の操作要素を列挙し（各要素に `data-dast-id` を付与、開いた shadow root も探索）、モデルは*セレクタではなく id で* 1 つ選び、`ClickLoop` がクリックして新しい状態を引き出します（あるいは長い/ページネーションされた一覧の行を読み込むためにスクロールし、プランナにより多くのオブジェクトを供給します）。このループは 3 つの方法で制限されます。ページ間で共有するクリック予算、要素の同一性（再付番される id ではなく）でキー付けした循環ガード、ページ内 DOM の変化を進捗とみなす空振りカウンタ。さらにモデルにはすでにクリックした要素が伝えられるため、繰り返さずに新たに現れた要素へと進みます。各クリックは `ClickGuard`（操作要素のアクセシブル名 / role に対する同じ破壊的パターン拒否リスト）で選別され、`ConsentGate`（許可ホストでのみ発火）でゲートされます。全体を通して、ブラウザはアプリが呼ぶ XHR / fetch エンドポイントを記録します。続いてプランナ（`IdorPlanner` / `ContentIdorPlanner`）が、**実際に観測した id から**、パラメータ・呼び出し元自身の値・候補 id（2 識別情報フローでは別アカウント由来）・ユーザーごとの判別フィールドを提案します。`IdorProbe` / `ContentIdorProbe` が決定的に確認します。呼び出し元自身の値でベースラインを取り、各候補を **呼び出し元として** 要求して返りを調べます。JSON API では、ユーザーごとの **判別フィールド** が呼び出し元自身と **異なる値で存在** すれば確定（=他人のレコード）。差分を取れるフィールドが無い HTML / SPA ページでは、**リークマーカー**——被害者の*データの指紋*——で確認します。`ContentIdor.markersFrom` が、*相手*アカウントのページには現れるが呼び出し元自身のページには現れない、短く特徴的なデータトークン（メールアドレス、ブランドのドメインなど）を抽出するため、共有の定型物（アプリ自身のブランド、CDN）は脱落し誤確定できません。検出には、その被害者固有のトークンが候補への応答に現れ、**かつ** 呼び出し元のベースラインには無いことが必要です（=被害者自身のデータが境界を越えた）。いずれの場合も、対象の id と一致するマーカーは却下されます（id はリークの有無に関わらずページに反射するだけで何も証明しない）ので、確認は*データ*に基づき、問い合わせた鍵そのものには基づきません。モデルは検出を捏造できず（堅牢なエンドポイントは差分を生まない）、破壊的操作も起こせません（拒否リストの最終防壁）。再現はパラメータ・値・フィールドで、モデルは不要です。
