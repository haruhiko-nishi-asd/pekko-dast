...
**はじめての方へ。** Web アプリは、実際に動いているときにだけ現れる形で、データを漏らしたり攻撃者に制御を渡したりすることがあります——細工リンクに追従してしまうページ、他人のレコードを返す API、どんなスクリプトからも読める Cookie など。本ツールは、あなたが所有する、またはテストを許可されたサイトに対して、注意深い攻撃者の役を演じます。実際のブラウザを駆動し、限られたメニューのプローブを（明示的に許可したホストに対してのみ）試し、実際に確認できたものだけを報告します——各検出には根拠と、正確に再現する手段が付き、単なる推測ではありません。再現可能で証拠に裏付けられた結果を求める開発者やセキュリティエンジニアのためのものです。知らない略語があれば、本文で使うものはすべて[用語集](#用語集)で説明しています。
...
ブラウザが必要な箇所（キャプチャ、XSS の実行、認証付き SPA（シングルページアプリ）のナビゲーションとログイン）では実際の Chromium を駆動し、不要な箇所（オープンリダイレクト、SQLi、SSRF、IDOR の確認）では素の HTTP を使います。全体を通じて 2 つの原則が貫かれています。1 つは **ホストを許可するまでは観測のみ**（能動的プローブは `DAST_AUTHORIZED_HOSTS` に対する `ConsentGate` でゲートされます）。もう 1 つは **モデルは提案するだけで、確認は決定的なコードが行う**（LLM は閉じたツールスキーマを埋めるだけで実行コードを書かず、決定的な確認なしに検出結果は生まれません）。実行方法は下記の「スキャンの実行」を参照してください。
...
本文で使う略語とセキュリティ用語の早見表です。馴染みがあれば読み飛ばしてください。

- **DAST** — *Dynamic Application Security Testing（動的アプリケーションセキュリティテスト）*：ソースコードを読むのではなく、**動作中**のアプリを外部から、利用者や攻撃者のように探査する手法。
- **決定的（deterministic）** — 同じ入力なら常に同じ結果になり、モデルが介在しないこと。完全に信頼できる検出結果です。
- **LLM** — *大規模言語モデル*（例: Claude）。ここでは固定メニューから次の一手を**提案する**だけで、実際の確認はすべて決定的なコードが行います。誤った推測が誤検出になることはありません。
- **XSS** — *クロスサイトスクリプティング*：被害者のブラウザに攻撃者のスクリプトを実行させること。**反射型**はリクエストに乗って届き（細工リンク）、**DOM ベース**はページ自身の JavaScript の中だけで起きます。
- **SQLi** — *SQL インジェクション*：ユーザー入力がデータベースのクエリに連結され、クエリ自体を書き換えてしまうこと。
- **SSRF** — *サーバサイドリクエストフォージェリ*：**サーバ自身**に、攻撃者が選んだ宛先（内部サービスやクラウドメタデータなど）へリクエストを送らせること。
- **SSTI** — *サーバサイドテンプレートインジェクション*：入力がデータとして扱われず、サーバのテンプレートエンジンで評価されてしまうこと。
- **IDOR** — *安全でない直接オブジェクト参照*：リクエスト中の id を変える（`?id=123` → `124`）と他人のデータが返ること。**壊れたアクセス制御**のよくある形です。
- **LFI / パストラバーサル** — *ローカルファイルインクルージョン*：本来読めないファイル（例: `../../etc/passwd`）をアプリに読ませること。
- **CORS** — *オリジン間リソース共有*：あるレスポンスをどのオリジンが読んでよいかというブラウザの規則。設定不備があるとサイトをまたいでデータが漏れ得ます。
- **JWT** — *JSON Web Token*：セッションに使う署名付きトークン。署名が弱い／無いと偽造できます。
- **CSRF** — *クロスサイトリクエストフォージェリ*：ログイン中ユーザーのセッションに便乗する偽造リクエスト。
- **SPA** — *シングルページアプリケーション*：UI を JavaScript（React、Vue など）で駆動するサイト。どのリンクも指していない背後の API を呼ぶことが多い。
- **XHR / fetch** — SPA がその背後の API を呼ぶために使うブラウザ API。
- **CDP** — *Chrome DevTools Protocol*：ここでは Cookie のフラグやストレージを読むために使う低レベルの通信路。
- **OAST** — *アウトオブバンドアプリケーションセキュリティテスト*：自分が制御するリスナーがコールバックを受け取るのを見て、「ブラインド」な脆弱性を確認する手法。
- **ソース / シンク / テイント** — **ソース**は攻撃者が影響し得る入力、**シンク**は危険な関数（`innerHTML` や `eval` など）、**テイント**追跡はデータがソースからシンクへ流れるかを見ること。
- **プローブ / ゲート / 観測のみ** — **プローブ**は細工した入力を送る能動的テスト。能動的プローブは**ゲート**され、許可したホストに対してのみ実行されます。それ以外は**観測のみ**（通常の訪問で既に見える情報を読むだけ）に留まります。
...
テンプレートをコピーして埋めます — `cp .env.example .env`。`DastConfig` がこれを `KEY=VALUE` として読みます（解決順: 実際の環境変数 → `.env` → JVM の `-D` プロパティ）。`export` は不要です。`.env` は gitignore 済みなので機密はここに置きます。`.env.example` に全キーをプレースホルダ付きで列挙しています。要点は次のとおり。

```dotenv
ANTHROPIC_API_KEY=sk-ant-...
DAST_AUTHORIZED_HOSTS=target.example   # 空 = 観測のみ（能動的プローブなし）
...
`scripts/` にコピーしてすぐ使える spec が 2 つあります。**`idor-spec.example.json`** は 3 通りの認証方法（`login` / `cookie` / `headers`）、`attacker`/`victim` の 2 識別情報構成、`cases` を網羅した注釈付きテンプレート。**`access-spec.example.json`** は同梱のデモ対象（`vuln-target.py`）に合わせた最小例です。どちらかを gitignore 対象の `*.local.json` にコピーし、実際の値を入れてください。
